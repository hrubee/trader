...
if vs_out="$(LOOP_TRADER_DATADIR="$REPO/loop_trader_data" timeout 180 "$PY" "$REPO/loop_trader.py" volspike --spike-tf 15m --top 20 --min-spike 8 --confirm-frac 1.0 2>>"$LOG")" && [ -n "$vs_out" ]; then
  printf '%s\n' "$vs_out" > "$SPIKES"
